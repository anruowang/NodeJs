...
function getGoodsInfoByPageOne(pageNo,pageSize){
    $.post("/getGoodsInfoByPageOne",{pageNo:pageNo,pageSize:pageSize},function(data){
        $.each(data,function(index,item){
            ...
            var str='<li><dl><dt><img src="'+pic+'"/></dt><dd class="goods_price">商品价格:&yen;'
                +item.price+'</dd><dd>商品名称:考点开放</dd>'+'<dd>商品类型:'+item.tname+'</dd></dl></li>';
            $("#goodsInfo").append($(str));
        });

        var total=parseInt(data.total);//获取记录数
        var totalPage=Math.ceil(total/pageSize);
        for(var i=1;i<=totalPage;i++){
            if(i==1){
                $("#pageInfo").append($('<li><a href="javascript:showGoodsInfoByPage('+i+','+
                    pageSize+')"class="checked">'+i+'</a></li>'));
            }else{
                $("#pageInfo").append($('<li><a href="javascript:showGoodsInfoByPage('+i+',' +
                    pageSize+')" class="unchecked">'+i+'</a></li>'))
            }
        }
        console.info(data);
    },"json");
...
//分页查询商品信息  pageNo:查第几页  pageSize:每页有多少条
    ...
    if(info!=""){
        $.get("checkUserName",{uname:info,tabname:tabName,colName:colName},function(data){
            ...
            if(data=="0"){
                ...
                $(obj).next().eq(0).text("用户名校验成功...").css("color","green");
            }else{
